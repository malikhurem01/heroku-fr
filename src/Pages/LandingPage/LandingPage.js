...
import sortByDate from "../../Utils/sortyByDate";
...
const LandingPage = () => {
  ...
  const sortProductsHandler = useCallback((state, response) => {
    setProductsState(sortByDate(state, response));
  }, []);

  useEffect(() => {
    productService.fetchAllProducts().then((response) => {
      if (response.data.length >= 1) {
        sortProductsHandler("start", response.data);
    ...
    <React.Fragment>
      ...
      <div>
        <div className={classes.products_tabs}>
          <p
            onClick={() => {
              sortProductsHandler("start", productsState);
          ...
          <p
            onClick={() => {
              sortProductsHandler("end", productsState);
