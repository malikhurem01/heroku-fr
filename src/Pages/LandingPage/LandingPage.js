...
const LandingPage = () => {
  ...
  const handleTabChange = (tab) => {
    if (tab === "NEW_ARRIVALS") {
      setProductsState(() => sort.sortByCreationDate(productsState));
      tabsHandler({ newArrivals: true, lastChance: false });
    } else if (tab === "LAST_CHANCE") {
      setProductsState(() => sort.sortByDaysRemaining(productsState));
  ...
  useEffect(() => {
    productService.fetchAllProducts().then((response) => {
      if (response.data.length >= 1) {
        setProductsState(() => sort.sortByCreationDate(response.data));
